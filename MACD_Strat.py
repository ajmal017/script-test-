...
# DEFINING MACD INDICATOR
    ...
    def next(self):
        ...
        if self.dataframe['macd_val'][self.nextCount] > self.dataframe['macd_signal_line'][self.nextCount] and \
                self.dataframe['macd_val'][self.nextCount] < 0 and self.dataframe['macd_signal_line'][self.nextCount] < 0 and \
                self.dataframe['Close'][self.nextCount] > self.sma1:
                self.buy()
                print('buy')


        if self.dataframe['macd_val'][self.nextCount] < self.dataframe['macd_signal_line'][self.nextCount] and self.dataframe['macd_val'][
            self.nextCount] > 0 and self.dataframe[
            'macd_signal_line'][self.nextCount] > 0 and self.dataframe['Close'][self.nextCount] < self.sma1:
            self.sell()
            print('sell')
